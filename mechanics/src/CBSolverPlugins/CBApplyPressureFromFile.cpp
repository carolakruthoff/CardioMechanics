//
//  CBApplyPressureFromFile.cpp
//  CardioMechanics
//
//  Created by ek717 on 24.04.2020.
//
//

#include "CBApplyPressureFromFile.h"
#include "CBSolver.h"
#include "CBElementSurfaceT3.h"
#include "CBElementSurfaceT6.h"

void CBApplyPressureFromFile::Init()
{
  // init filenames
  std::string defaultFilename = adapter_->GetSolver()->GetModel()->GetExporter()->GetExportDir() + "/ApplyPressureFromFile.dat";
  filenameOut_ = parameters_->Get<std::string>("Plugins.ApplyPressureFromFile.ExportFile", defaultFilename);

  // filenameIn_ = parameters_->Get<std::string>("Plugins.ApplyPressureFromFunction.PressureFile", "PressureFile.dat");

  // init cavities map using function from base class
  // CBSolverPluginCavities::InitCavities("Plugins.ApplyPressureFromFunction");
    CBSolverPluginCavities::InitCavities("Plugins.CBApplyPressureFromFile");

  // init groups and intervals
  std::vector<std::string> groupKeys = parameters_->GetChildNodes("Plugins.ApplyPressureFromFile.Groups");
  for (auto &groupKey : groupKeys)
  {
    ...
    if (intervalKeys.size() == 0)
      throw std::runtime_error("void CBApplyPressureFromFile::Init(): No Intervals specified in " + groupKey);
    ...
    for (auto &intervalKey : intervalKeys)
    {
      ...
      // interval.pressureFileName = parameters_->Get<std::string>(intervalKey + "PressureFile", "PressureFile.dat");
      interval.pressureFile  = std::make_shared<CBDataFromPressureFile>(parameters_, intervalKey + ".PressureFromFile");
      ...
      if (interval.startTime < prevStopTime)
        throw std::runtime_error("void CBApplyPressureFromFile::Init(): StartTime of " + intervalKey + " is smaller than StopTime of previous Interval");
    ...
    for (auto &surface : surfaces)
    {
      if (cavities_.find(surface) == cavities_.end())
        throw std::runtime_error("void CBApplyPressureFromFile::Init(): No cavity with surface index " + std::to_string(surface) + " found.");
...
void CBApplyPressureFromFile::Apply(TFloat time)
{
    ...
    for (groupsIt_ = groups_.begin(); groupsIt_ != groups_.end(); groupsIt_++)
    {
        for (auto &interval : groupsIt_->second)
        {
            ...
            if (time >= interval.startTime && time < interval.stopTime+0.5*dt)
            {
                ...
                // input file is in mmHg and pressure needs tobe in Pa
                TFloat f = interval.pressureFile->Get(time,0) * 133.322;
                // ek717 Todo write a CBPressureFromFile : public CBData wie CBDataFromFIle.....
                if (interval.invert)
...
void CBApplyPressureFromFile::ApplyToNodalForces()
{
...
void CBApplyPressureFromFile::ApplyToNodalForcesJacobian()
{
   for (valuesIt_ = valuesStructs_.begin(); valuesIt_ != valuesStructs_.end(); valuesIt_++)
...
void CBApplyPressureFromFile::AnalyzeResults()
{
    for (valuesIt_ = valuesStructs_.begin(); valuesIt_ != valuesStructs_.end(); valuesIt_++)
...
void CBApplyPressureFromFile::WriteHeaderToFile()
{
  fileOut_.open(filenameOut_.c_str());
  if (!fileOut_.good())
    throw std::runtime_error("void CBApplyPressureFromFile::WriteToFile(TFloat time): Couldn't create " + filenameOut_);
  fileOut_ << std::setw(16) << "time";
  for (valuesIt_ = valuesStructs_.begin(); valuesIt_ != valuesStructs_.end(); valuesIt_++)
  {
    std::ostringstream p, v;
    p << "pressure" << valuesIt_->first;
    v << "volume" << valuesIt_->first;
    fileOut_ << std::setw(16) << p.str() << std::setw(16) << v.str();
  }
  fileOut_ << std::endl;
  fileOut_.close();
}
...
void CBApplyPressureFromFile::WriteToFile(TFloat time)
{
  ...
  // write plugin data
  fileOut_.open(filenameOut_.c_str(), std::ios::app);
  if (!fileOut_.good())
    throw std::runtime_error("void CBApplyPressureFromFile::WriteToFile(TFloat time): Couldn't create " + filenameOut_);
  fileOut_ << std::setprecision(7) << std::fixed << std::setw(16) << time;
  for (valuesIt_ = valuesStructs_.begin(); valuesIt_ != valuesStructs_.end(); valuesIt_++)
  {
    fileOut_ << std::setw(16) << valuesIt_->second.pressure/133.322 << std::setw(16) << (valuesIt_->second).volume*1e6;
